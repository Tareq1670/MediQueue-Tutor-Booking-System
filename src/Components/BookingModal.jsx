    ...
    useEffect(() => {
        if (tutor?.startDate) {
            ...
            if (currentDate > startDateTime) {
                setError("Booking session has expired! Please try another available session.");
            } else {
                setError("");
            }
        }
                        ...
                        <Modal.Dialog className="w-full max-w-lg bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800/60 rounded-lg shadow-2xl p-2 m-4">
                            <Modal.Header className="flex flex-col items-center justify-center text-center pt-8 pb-2 px-6">
                                ...
                                <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-2 max-w-[280px] leading-relaxed font-normal">
                                    Fill out the details below to secure your medical learning slot.
                                </p>
                            ...
                            <Modal.Body className="p-6">
                                <Surface variant="default" className="bg-transparent shadow-none border-none p-0">
                                    <Form onSubmit={handleSubmit} className="flex flex-col gap-4">
                                        <TextField isRequired name="name" type="text" defaultValue={user?.name} className="w-full">
                                        ...
                                        <TextField isRequired name="tutorName" type="text" defaultValue={tutor?.name || "TARIQUL ISLAM TAREQ"} className="w-full">
                                            <Label className="text-xs font-semibold uppercase tracking-wider text-zinc-600 dark:text-zinc-400 mb-1.5 block">
                                                Tutor Name
                                            </Label>
                                        ...
                                        <TextField isRequired name="email" type="email" defaultValue={user?.email} className="w-full">
                                            <Label className="text-xs font-semibold uppercase tracking-wider text-zinc-600 dark:text-zinc-400 mb-1.5 block">
                                                Email
                                            </Label>
                                            <Input className="w-full h-12 px-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50 text-sm text-zinc-800 dark:text-zinc-200 focus:border-emerald-500 dark:focus:border-emerald-500 focus:outline-none transition-all duration-200 shadow-none" />
                                        ...
                                        <div className="flex justify-end items-center gap-3 pt-4">
                                            <Button slot="close" variant="secondary" className="px-5 h-11 rounded-xl bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200/70 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 text-xs font-bold tracking-wide active:scale-[0.98] transition-all duration-200">
                                                Cancel
                                            </Button>
                                            ...
                                            >
                                                {error ? "Expired" : exit ? "Already Booked" : loading ? "Booking..." : "Confirm Booking"}
                                            </Button>
                                        </div>
                                    </Form>
                                </Surface>
